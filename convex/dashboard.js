...
export const getUserBalances = query({
  handler: async (ctx) => {
    ...
    /* ───────────── All expenses (both 1‑to‑1 and group) ───────────── */
    ...
    /* ───────────── All settlements (both 1‑to‑1 and group) ───────────── */
    ...
    for (const [uid, { owed, owing }] of Object.entries(balanceByUser)) {
      ...
      if (Math.abs(net) < 0.005) continue; // Skip only if truly zero (accounting for floating point)
      ...
      const base = {
        ...
        amount: Math.round(Math.abs(net) * 100) / 100, // Round to 2 decimal places
      };
    ...
    // Recalculate totals from the actual lists to ensure consistency
    const recalculatedYouOwe = youOweList.reduce((sum, item) => sum + item.amount, 0);
    const recalculatedYouAreOwed = youAreOwedByList.reduce((sum, item) => sum + item.amount, 0);
    ...
    console.log("Balance calculation debug:", {
      ...
      youAreOwedByList,
      recalculatedYouOwe,
      recalculatedYouAreOwed,
      totalBalance: recalculatedYouAreOwed - recalculatedYouOwe
    });

    return {
      youOwe: Math.round(recalculatedYouOwe * 100) / 100, // Round to 2 decimal places
      youAreOwed: Math.round(recalculatedYouAreOwed * 100) / 100,
      totalBalance: Math.round((recalculatedYouAreOwed - recalculatedYouOwe) * 100) / 100,
