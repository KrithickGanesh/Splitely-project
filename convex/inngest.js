...
export const getUsersWithOutstandingDebts = query({
  handler: async (ctx) => {
    ...
    // Load all expenses (both 1-to-1 and group expenses)
    const expenses = await ctx.db
      .query("expenses")
      .collect();

    // Load all settlements (both 1-to-1 and group settlements)
...
export const getUsersWithExpenses = query({
  handler: async (ctx) => {
    ...
    // Get expenses from the past 3 months (more inclusive for AI insights)
    const now = new Date();
    const threeMonthsAgo = new Date(now);
    threeMonthsAgo.setMonth(now.getMonth() - 3);
    const monthStart = threeMonthsAgo.getTime();
...
// Get a specific user's expenses for AI analysis
export const getUserMonthlyExpenses = query({
  args: { userId: v.id("users") },
  handler: async (ctx, args) => {
    // Get expenses from the past 3 months for better AI analysis
    const now = new Date();
    const threeMonthsAgo = new Date(now);
    threeMonthsAgo.setMonth(now.getMonth() - 3);
    const monthStart = threeMonthsAgo.getTime();

    // Get all expenses involving this user from the past 3 months
